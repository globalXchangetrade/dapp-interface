...
export const Popup = (props: any) => {
  ...
  const [defaultOverflow, setDefaultOverflow] = useState<any>()
  ...
  const checkRemoveBackdrop = () => {
    const modals: any = document.querySelectorAll('.popup-component')
    /**
    ...
    if (isFirst) {
      ...
      if (defaultOverflow) {
        (window as any).document.body.style.overflow = defaultOverflow
      }
      (window as any).document.body.style.paddingRight = defaultPaddingRight
    }
  }

  useEffect(() => {
    if (open) {
      let modalRoot: any = window.document.getElementById('app-modals')
      if (!modalRoot) {
        ...
        setRoot(modalRoot)
        setIsFirst(true)
        setDefaultOverflow((window as any).document.body.style.overflow)
        setDefaultPaddingRight((window as any).document.body.style.paddingRight)
      } else {
        setRoot(modalRoot)
      }
      if (modalRef.current) {
    ...
    <>
      {
        open && root && ReactDOM.createPortal(
          <div className='popup-component' style={popupStyles} onClick={handleClick} onKeyDown={handleKeyDown} tabIndex={-1} ref={modalRef}>
            {
